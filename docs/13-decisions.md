...
## D-067：追加5个百分点预算完成里程碑5，区分受托与亲自验收

- 日期：2026-09-12。
- 状态：Accepted，基于用户最新“完成里程碑5，预算为5%用量”。
- 决策：本次从账户已用9%起，新增最多约5个百分点；完成LabLedger独立审查、源码核验及本地可移动交付后停止。正式公开发布和全局安装升级仍属里程碑6。
- 证据：最终审查接收绑定当前commit的原始浏览器检查及协作记录，它们是待核对证据，不是审查指令。只读审查不通过重跑需要写目录的测试来替代有效沙箱Gate回执。
- 验收者：本次主任务依据用户完整委托执行操作者验收；accept_checkpoint明确记delegated-operator，保留用户亲自接受与受托验收的区别，不能伪称用户已亲自查看页面。

## D-068：恢复预算、源码残留和阻塞恢复的收口规则

- 日期：2026-09-12；状态：Accepted。
- 独立源码审查确认三个P2：原生恢复后total重置会低估预算，Git忽略文件可漏过所有权检查，已停止的blocked工作包缺少恢复路径。
- 用量：保存turnId和每turn累计高水位；同一CAS内对首次观察的增量累计并去重。旧缺字段历史不推算补齐，明确legacy可观测性不足。账户额度仍由主任务独立监控，原生tokens与账户百分比不是同一指标。
- 边界：枚举包含ignored文件；Git读取警告不能解释成clean。预留.team-temporary和验证为owned源码编译产物的字节码可作为运行残留；未知文件保留并阻塞，不自动删除。Git内部元数据的全面防篡改不属于本版保证。
- 恢复：先保存完成回执，再进行源码验收。reconcile对running/blocked旧会话核对idle、空背景终端和当前源码后转paused，允许继续原Session；idle不等于验收通过，session替换仍要求可信中断回执。
- 实际收口：LabLedger审查review-46e5615928a8.json已approved，全部11条要求满足；候选4663e17e不变。四个本实验测试残留目录通过原生workspaceWrite沙箱保存在.team-temporary/preserved-test-residue-20260912，未删除。12:06 UTC执行delegated-operator接受，未冒称个人视觉接受；里程碑6保持未启动。

## D-069：暂停既有路线，保留方向调整前的完整基线

- 日期：2026-09-13；状态：Accepted，用户最新明确指令。
- 决策：立即停止推进，仅核对、记录和提交最新状态。里程碑1–5源码基线a911dbd及其原始实验、验收、交付包保留；不启动里程碑6或新实验。
- 待定：用户将调整方向，但尚未提供具体变更；不自行推断新的目标、架构或范围。收到新方向后再判断哪些决策被替代，历史结论不追溯改写。
- 预算：此前5个百分点执行预算已结束，本次保存记录不恢复任何模型实验。

## D-070：采用新版方向及用户四项补充

- 日期：2026-09-14；状态：Accepted，来源为用户主任务中的最新明确补充。
- 方向：以用户提供的项目定义v0.2为方向依据，并以最新四项补充修订：带信息与选项的主动澄清、跨领域任务承接及必要联网查证、效率质量自主性的约束平衡、Codex Plugin作为产品分发单位。
- 边界：Codex-only继续有效，任务领域不再限于软件工程；Team核心不绑定第三方Skill/插件。现有代码与里程碑验收保留其原适用范围，不据此宣称新方向已实现。
- 依据与建议分别记录于docs/24-direction-update-2026-09-14.md；阈值、交互细节、约束分层及实现选型仍待设计与验证。
- 衔接：D-069中等待方向说明的事项已有输入，进入定义与设计更新；不沿用已结束的5个百分点模型实验预算，不因此自动安装或发布插件。

## D-071：保留模型沟通自由度，主动演化正式定义并保持协作可读

- 日期：2026-09-14；状态：Accepted。来源：用户三项修正、对主任务修订理解的“可以”确认。
- 沟通：组织方式在理解与调查后作为正式运行前的执行方案建议，不要求用户在入口选模式。不固定首轮回答、提问顺序或回复结构，选项、逻辑性段落等由模型按内容选择；系统支持模型提出自己的判断，由用户评估反馈。
- 定义：完整连贯叙述作为正式目标描述，讨论中的新想法可先形成草案。用户明确修订及已采纳共识由模型主动整理并合入，不逐次索要写文档许可；不能把沉默或试探性讨论当采纳。工作笔记作为导航，版本与原始记录保留依据。
- 通信：Agent交流默认让用户能够理解，正文不采用仅供Agent理解的压缩语言。A2A作为规范化交流的参考，程序元数据与自然语言正文相辅相成；实际协议映射未因此视为已实现。
- 当前正式入口为docs/project-definition.md（定义v0.3，非插件版本）；docs/23保留用户原文，docs/24保留补充来源。D-070中的“带信息与选项”按本条解释，不形成每轮选项模板。
- 后续设计重点转向可靠执行、队列、状态及恢复，不把固定化澄清体验设计作为前置条件。本次只更新定义及记录，不恢复已结束预算的模型实验。

## D-072：采用自主执行、渐进计划与可追踪协作的设计

- 日期：2026-09-14；状态：Accepted。来源：用户对下一步设计建议明确回复“就按你的建议设计吧”。
- 已接受原则：总体协调责任持续明确且可交接，局部执行充分自主；模型负责语义判断与组织，程序保存并落实确定的动作；计划随调查展开，跨范围调整按影响版本化，不逐次重审已有授权。
- 协作：纯信息保留可读消息，行动请求进入统一工作记录；工作与Session、执行尝试分离。普通请求排队，不默认注入忙碌回合；接收结果和完成执行分别记录，按领域和风险选择验收。
- 主任务据此采用的实现设计：声明式工作记录及受控变更；工作、分配epoch、Attempt和Acceptance分别建模；多对象事务、事件及outbox一致提交；停止请求与已停事实分离，未知派发和副作用先核对再重试。
- 设计文件：docs/25-adaptive-execution-design.md及docs/26-execution-validation-and-migration.md，含权限、状态、迁移、两条完整场景和EX-01至EX-22验证计划。详细字段与能力界限不是用户逐项指定，也尚未实测。
- 适用：正式定义更新至v0.4；docs/06与docs/21中的旧profile/0.2契约仅按其原版本解释，不将旧完成状态升级为新版接收证据。旧数据、已完成里程碑和原文保留。
- A2A：本轮核对1.0.0官方规范及任务生命周期，采用设计层语义参考；本地接收、责任与版本控制由Team负责，不新增第三方依赖、不声称线协议互操作。
- 边界：本轮为设计委托，未修改运行代码、未进行模型实验或安装发布。后续实测沿用有效模型约定，但不得沿用已经结束的额度。


## D-073：进入0.3自适应插件预览实现

- 日期：2026-09-14；状态：Accepted。依据：用户授权正式制作plugin，并明确本轮实验最多5个百分点、Luna/max。
- 新入口team-next与0.3状态结构分开实现，复用CodexClient及SQLite基础；旧0.2数据不改写，不将旧completed转换成新accepted。
- 对话自由度留给模型，完整定义与工作笔记/原始历史延续D-071/D-072。模型提出协作/计划/接收判断，程序事务化落实；普通消息不默认steer忙碌回合。
- 原生独立会话先行，内部Subagent暂禁用；全局跨state锁、自动会话替换、通用外部动作账本、局部计划自动rebase未完成，不作能力保证。完整定义更新先在确认边界执行。
- 实测观察、修正和验证缺口见docs/27-adaptive-plugin-preview.md；该文明确区分程序检查、原生实验和未运行设计情形。本轮仅交付预览，不发布公开Release，也不声称完整可靠性验收通过。
