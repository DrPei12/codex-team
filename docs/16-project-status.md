...
## 0.3当前工作状态（2026-09-14）

**已进入0.3预览实现及真实验证，尚未认定完整设计验收通过。** 当前方向为[项目定义v0.4](project-definition.md)，D-073记录实现边界；代码、验证结果和剩余缺口集中见[预览实现记录](27-adaptive-plugin-preview.md)。新工作/尝试/接收结构、SQLite命令事务、原生执行队列、局部变更、停止核对及看板已连通；新模型实验获得最多5个百分点授权，统一Luna/max。原[验证及迁移计划](26-execution-validation-and-migration.md)保留为设计时基线，实际执行状态以新的证据为准。
