...
截至 2026-08-26，上述六个 phase skill 与统一 `$team` 只读路由都有 v0.1 实现。`team-plan` 已在 `main`，其余位于 `codex/team-v01` stacked branch。确定性 builder 已将整套构建为可移动 skills-only plugin，repo marketplace contract 进入测试；九组共 99 项测试。`codex-team@codex-team-local` 在 Codex CLI `0.146.0` 中完成两次安装/重安/卸载，新任务发现并显式加载 7 个 skill，一条高匹配请求隐式选中总入口，两条卸载后新任务均不再发现 plugin。长期触发准确率、旧会话热刷新、版本升级/cachebuster、Desktop worker dispatch/live observation/handoff/archive、第二 blind benchmark 和正向边际效用仍未验证，因此整套保持 `incubating`。
...
文件 SHA-256。后续真实安装已证明当前 Codex 新任务能从 plugin cache 保留该布局并加载全部 7 个 skill；这仍是单机/单版本观察。
...
生成时注入 + bundled `team` core skill 已在仓库外临时目录和真实 Codex plugin cache 中运行通过，因此不再把“包内资源定位”或“新任务是否发现 7 个 skill”列为未解问题。仍需实测的是长期触发/非触发准确率、旧任务热刷新、版本升级/cachebuster、禁用与非同版卸载语义。
...
8. 已实现统一 `team` 只读路由、离线端到端验收、可移动 skills-only plugin builder、repo marketplace 与真实安装/发现/显式加载/卸载验收；下一步是只读 live observer、最小 Desktop worker pilot、版本升级语义和第二 blind benchmark；
