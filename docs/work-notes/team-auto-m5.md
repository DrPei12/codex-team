...
当前正式方向见[项目定义v0.3](../project-definition.md)：跨领域Codex Plugin，按内容自由沟通、主动提出判断，组织方案在正式运行前建议；完整定义随草案共识主动演化，Agent交流保持用户可读。原始方向及补充通过定义引用检索，最新决策D-070/D-071。后续重点讨论执行、队列、状态与恢复，不先固化澄清脚本。模型实验保持停止，不沿用已结束额度。
