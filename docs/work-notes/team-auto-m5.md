...
当前正式方向见[项目定义v0.4](../project-definition.md)，最新决策D-072。用户已接受执行机制建议并委托设计；[执行设计0.1](../25-adaptive-execution-design.md)与[验证及迁移计划](../26-execution-validation-and-migration.md)已形成，包含权限、动态工作/队列、结果接收、事务派发、局部变更和恢复。原始讨论见docs/15最新条目，代码对照基线485485a。两条完整场景及EX-01至EX-22均未运行；下一实现入口是命令事务、工作/尝试/接收与outbox，再接真实Codex队列和恢复。模型实验保持停止，不沿用已结束额度。
