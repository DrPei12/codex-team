...
当前阅读入口是正式项目定义、执行机制设计和当前状态。读者按需要进入实现契约、历史讨论与证据，无需一次加载全部内容。

最后同步：2026-09-15。当前采用正式定义v0.4和0.3预览实现（D-073）；实际能力、安装、实验及未完成验收见[预览记录](27-adaptive-plugin-preview.md)。本轮授权的5个百分点实验预算已结束，后续不得自动恢复。0.2里程碑1–5的完成与原适用证据见[验收记录](22-milestone5-acceptance.md)。下列0.1专题、模板和早期验收保留历史语境；与当前方向冲突时按最新用户指令、已接受决策和新设计处理。

## 第一层：五分钟了解项目

- [正式项目定义](project-definition.md)：当前目标、用户意愿、核心关系与行为原则。
- [自适应执行设计](25-adaptive-execution-design.md)：协调权限、工作模型、队列、结果接收和恢复。
- [验证与实现衔接](26-execution-validation-and-migration.md)：旧实现差距、首个切片、验收场景与迁移。
- [0.3实现与验证](27-adaptive-plugin-preview.md)：已安装的预览能力、实测修正、证据与限制。
- [当前工作笔记](work-notes/adaptive-plugin.md)：继续位置、停止边界与原始历史导航。
- [原始项目章程](00-project-charter.md)：历史目标、边界和成功标准。
- [概念模型](01-conceptual-model.md)：角色、编排范式、历史来源、workspace 和生命周期为什么不能混为一谈。
- [早期运行架构](04-default-operating-model.md)：保留当时的团队与流程设计，当前组织方式由任务决定。
