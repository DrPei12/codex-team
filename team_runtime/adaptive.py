...
class Adaptive:
    ...
    def create(self, workspace, definition, works, *, authority, policy=None, operation_id=None, title="Team项目"):
        ...
        settings = validate_policy({**(policy or {}), "network_access": (policy or {}).get("network_access", True)})
    ...
    def eligible(data):
        ...
        return sorted(eligible, key=lambda w: (-w["priority"], w["created_at"], w["id"]))
    ...
    def prepare(self, run_id, work_id, owner, epoch, *, operation_id=None):
        ...
        def change(tx, data):
            ...
            for attempt in active:
                if attempt["role"] == work["role"]:
                    raise ConflictError("Role already has an active responsibility")
                if work["writable"] and attempt["writable"] and overlap(cwd, attempt["cwd"]):
    ...
    def prepare_coordination(self, run_id, owner, epoch):
        def change(tx, data):
            ...
            if any(a["state"] in ACTIVE_ATTEMPTS for a in data["attempts"].values()):
                raise ConflictError("Coordinate at a confirmed boundary")
            if data["usage"]["observed_tokens"] >= data["policy"]["token_budget"]:
            ...
            attempt = {"id": attempt_id, "work_id": None, "role": "team-coordinator",
                       "cwd": data["workspace"], "writable": False, "assignment_epoch": epoch,
                       "controller_epoch": epoch, "state": "prepared", "thread_id": None,
                       "native_phase": "not-started",
                       "turn_id": None, "definition": data["definition"], "created_at": _utc_now()}
            data["attempts"][attempt_id] = attempt
            tx.put("adaptive-dispatch", attempt_id, {"run_id": run_id, "state": "pending", "attempt_id": attempt_id})
            return {"attempt": attempt}
    ...
    def bind(self, run_id, attempt_id, *, thread_id, turn_id=None, configuration=None):
        def change(tx, data):
            ...
            if turn_id:
                ...
                if work:
                    work["state"] = "working"
            key = digest({"role": attempt["role"], "cwd": attempt["cwd"], "writable": attempt["writable"]})
    ...
    def request(self, run_id, attempt_id, target_work, question, *, wait=True, operation_id=None):
        ...
        def change(tx, data):
            ...
            if not target or target_work == source["id"]:
    ...
    def accept(self, run_id, work_id, result_hash, rationale, *, actor="coordinator", operation_id=None):
        text(rationale, "rationale")
        self.validate_result(run_id, work_id, result_hash)
        if actor not in {"coordinator", "operator", "user"}:
        ...
        def change(tx, data):
            ...
            record = {"id": ident("acceptance"), "work_id": work_id, "work_revision": work["revision"],
                      "result": work["result"], "criteria": work["acceptance"], "actor": actor,
    ...
    def revise(self, run_id, base_revision, reason, *, add=None, update=None, definition=None, operation_id=None):
        text(reason, "reason")
        ...
        def change(tx, data):
            ...
            for patch in update or []:
                ...
                if set(patch) - {"id", "goal", "acceptance", "depends_on", "priority", "role", "state", "note"}:
        ...
        return self._mutate(run_id, operation_id or ident("change"), "plan-revised", payload, change, actor="coordinator")
    ...
    def settle(self, run_id, owner, epoch, *, elapsed=0):
        def change(tx, data):
            ...
            return {"status": data["status"]}
        return self._mutate(run_id, ident("settle"), "controller-settled", {"elapsed": elapsed}, change)
    ...
    def reconcile_observations(self, run_id, observations, snapshot_ref):
        ...
        return self._mutate(run_id, ident("reconcile"), "run-reconciled", {"snapshot": snapshot_ref}, change)
