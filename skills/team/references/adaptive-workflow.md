...
Use this for new delegated goals and adaptive state schema 0.3. Existing 0.2 Auto
...
(relative to WORKSPACE, default `.`), `writable` (default false), `depends_on`
...
coordination and verification in resource accounting. Current local defaults are
experimental choices, not universal optimal settings. The adaptive runner currently
uses independent sessions with internal Subagents disabled; do not promise that
changing their ceiling enables an unqualified lifecycle.
