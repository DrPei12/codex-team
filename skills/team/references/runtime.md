...
Use these operations as needed for the current assignment. They share one
runtime and state schema 0.3; their order follows the work and its evidence.

## Locate the runtime and preserve context

Resolve `TEAM` to `scripts/team.py` beside the loaded `team/SKILL.md`. Use an
...
Run `python -B TEAM --state STATE create --workspace WORKSPACE --definition-file
...
Run `python -B TEAM --state STATE run RUN_ID` in a persistent terminal. It performs
...
`snapshot RUN_ID` reads current facts; `list` finds existing Team runs.
