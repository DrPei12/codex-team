...
运行程序使用 Python 3.12+ 与标准库，开发检查安装 `pytest`。真实模型验证使用独立工作区与状态目录，明确选择模型和预算。
...
发布检查运行运行时、当前状态续接与打包测试，构建新包、校验清单和导入，并与仓库内分发包比较。回归测试验证真实状态变化或用户结果；原生测试替身明确标注，执行行为变化同时进行真实 Codex 验证。
